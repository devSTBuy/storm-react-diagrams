...
export class DiagramWidget extends BaseWidget<DiagramProps, DiagramState> {
	...
	startFiringAction(action: BaseAction) {
		var setState = true;
		if (this.props && this.props.actionStartedFiring) {
			...
			if (!this.props.allowLooseLinks && this.state.wasMoved) {
				_.forEach(this.state.action.selectionModels, model => {
					...
					if (
						...
						link.getSourcePort().id === link.getTargetPort().id
					) {
						link.remove();
					}
				});
			  ...
			  */
				...
				onMouseDown={event => {
					...
					if (model === null) {
						...
						let selectedWorkTables: any[] = selectedItems.filter(item => item.type === 'work-table');
						let editable = selectedWorkTables.filter(item => item.editable);
						if (editable.length === 0 && model.model.type === 'work-table') {
							diagramModel.clearSelection();
							this.startFiringAction(new MoveCanvasAction(event.clientX, event.clientY, diagramModel));
						}
					}
